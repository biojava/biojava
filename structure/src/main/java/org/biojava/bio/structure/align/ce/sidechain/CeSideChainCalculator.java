...
public class CeSideChainCalculator  {
	...
	public void extractFragments(CeParameters params, AFPChain afpChain,
		...
		// no idea so far what f1, f2 are being useful for.
		// seems they are needed in traceFragmentMatrix
		mainLoop1:
			for(int is=0; is<nse1-winSize+1; is++) {
				f1[is]=0;
				for(int ir=0; ir<winSize; ir++){ 
					if(ca1[is+ir].getX()>=1e10){
						f1[is]=1;
						continue mainLoop1;
					}		    	
				}	
		...
		mainLoop2:
			...
			dist1 = new double[nse1][nse1];
			dist2 = new double[nse2][nse2];

			// build up intramolecular distance matrix dist1;
			for(int ise1=0; ise1<nse1; ise1++)  {

				for(int ise2=0; ise2<nse1; ise2++)  {
					double dist = 2e10;
				
					dist = Calc.getDistance(ca1[ise1], ca1[ise2]);
								
					dist1[ise1][ise2] = dist;			
				}
			}

			// build up intramolecular distance matrix dist2
			for(int ise1=0; ise1<nse2; ise1++) { 
				for(int ise2=0; ise2<nse2; ise2++) {
					double dist = 2e10;
					if ( ca2[ise1].getX()<1e10&&ca2[ise2].getX()<1e10 ) {
						dist =Calc.getDistance(ca2[ise1], ca2[ise2]);
					}

					dist2[ise1][ise2] = dist;
...
// AP: trying to use a different way to calculate AFPs				
//				double d1 = 0;
//				
//				if ( ise1+3 < nse1 && ise2+3<nse2)
//					d1 = Math.abs(dist1[ise1+3][ise1+3]-dist2[ise2+3][ise2+3]);
//				double d2 = 0;
//				if ( ise1+8 < nse1 && ise2+8<nse2)
//					d2 = Math.abs(dist1[ise1+8][ise1+8]-dist2[ise2+8][ise2+8]);
//				
//				mat[ise1][ise2]=d1 +d2;
...
nBestTrace=nTrace;
	...
	private void checkBestTraces(CeParameters params, AFPChain afpChain,
		...
		String[] alignmentAtoms = params.getAlignmentAtoms();
		Atom[] strBuf1 = new Atom[traceMaxSize*alignmentAtoms.length];
		Atom[] strBuf2 = new Atom[traceMaxSize*alignmentAtoms.length];
		...
		if ( debug ){			
			checkPrintRmsdNew(traceMaxSize, winSize, ca1, ca2, alignmentAtoms);						
		}
		...
		for(int ir=0; ir<nBestTraces; ir++) {
			...
			rmsdNew = getRMSDForBestTrace(ir, strBuf1, strBuf2, bestTracesN,bestTraces1, bestTrace2,winSize,ca1,ca2, alignmentAtoms);
		...
		for(jt=0; jt<nBestTrace; jt++) {
			for(int i=0; i<bestTraceLen[jt]; i++) {
				setStrBuf(strBuf1,is+i,ca1,bestTrace1[jt]+i, alignmentAtoms);
				setStrBuf(strBuf2,is+i,ca2,bestTrace2[jt]+i, alignmentAtoms);
		...
		if(z>=zThr) {
			nGaps = optimizeSuperposition(afpChain,nse1, nse2, strLen, rmsd, ca1, ca2,nGaps,strBuf1,strBuf2, alignmentAtoms);
			//	      if(isPrint) {
			//		/*
	 ...
	 * @param j
	 */
	private void setStrBuf(Atom[] strBuf1, int i, Atom[] ca1, int j, String[] alignmentAtoms) {
		// TODO Auto-generated method stub
		//TODO
		Group parent = ca1[j].getParent();
		int pos = 0;
		for (String atomname : alignmentAtoms ) {
	...
	// TODO: needs to consider all requested Atoms
	private double getRMSDForBestTrace(int ir, Atom[] strBuf1, Atom[] strBuf2,
			int[] bestTracesN2, int[][] bestTraces12, int[] bestTrace22, 
			int winSize,Atom[] ca1, Atom[] ca2, String[] alignmentAtoms ) throws StructureException {
		int is=0;
		for(int jt=0; jt<bestTracesN[ir]; jt++) {
			for(int i=0; i<winSize; i++) {

				setStrBuf(strBuf1, is+i, ca1, bestTraces1[ir][jt]+i, alignmentAtoms);
				setStrBuf(strBuf2, is+i, ca2, bestTraces2[ir][jt]+i, alignmentAtoms);
	 ...
	 * 
	 */
	private void checkPrintRmsdNew(int traceMaxSize, int winSize, Atom[] ca1, Atom[] ca2, String[] alignmentAtoms) throws StructureException{
	...
	private int optimizeSuperposition(AFPChain afpChain, int nse1, int nse2, int strLen, double rmsd, Atom[] ca1, Atom[] ca2,int nGaps, Atom[] strBuf1, Atom[] strBuf2 , String[] alignmentAtoms) throws StructureException {
		...
		while(nAtom<strLen*0.95 || 
			...
			for(int ia=0; ia<lcmp; ia++) {
				if(align_se1[ia]!=-1 && align_se2[ia]!=-1) {
					setStrBuf(strBuf1, nAtom, ca1, align_se1[ia], alignmentAtoms);
					setStrBuf(strBuf2, nAtom, ca2, align_se2[ia], alignmentAtoms);
					nAtom++;
	...
	// no need to consider all requested atoms, since we are just iterating over the whole group
