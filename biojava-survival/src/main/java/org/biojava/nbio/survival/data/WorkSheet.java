...
public class WorkSheet {
	...
	private final LinkedHashMap<String, String> metaDataColumnsHashMap = new LinkedHashMap<>();
	...
	private final LinkedHashMap<String, String> metaDataRowsHashMap = new LinkedHashMap<>();
	...
	public String getCell(String row, String col) throws Exception {
		...
		if (rowIndex == null) {
			...
			for (Map.Entry<String, HeaderInfo> entry : rowLookup.entrySet()) {
				if (row.equalsIgnoreCase(entry.getKey())) {
		...
		if (colIndex == null) {
			//allow for case insentive search
			for (Map.Entry<String, HeaderInfo> entry : columnLookup.entrySet()) {
	...
	public void changeColumnsHeaders(LinkedHashMap<String, String> newColumnValues) throws Exception {
		for (Map.Entry<String, String> entry : newColumnValues.entrySet()) {
			String newColumn = entry.getValue();
			changeColumnHeader(entry.getKey(), newColumn);
		}
			...
			for (String col : columns) {
				if (this.isMetaDataColumn(col) || this.isMetaDataRow(row)) {
					...
					try {
						...
						workSheet.addCell(row, col, String.valueOf(d));
					} catch (Exception e) {
						workSheet.addCell(row, col, value);
					}
