...
/**
 * Refines an alignment with open symmetry, that is, with tanslational and
 * rotational components in the symmetry operation, so that the aligned residues
 * are consistent.
 * <p>
 * This refiner also determines the order of symmetry (number of repeats), by
 * the heuristic: the order that contains maximum number of residues aligned.
 * <p>
 * The goal is to find the internal repeats without ths constraint that the
 * first and last subunit align, so no CP has to be present in the CeSymm
 * alignment.
 * <p>
 * The symmetry axis indicates also the translation of the subunits in this
 * case. It does not work for all close symmetry cases, because it assumes that
 * no CP ocurred in the first alignment.
 * 
 * @author Aleix Lafita
 * @since 4.2.0
 * 
...
public class OpenRefiner implements SymmetryRefiner {
	...
	public MultipleAlignment refine(AFPChain selfAlignment, Atom[] atoms, int order)
		...
		List<List<Integer>> graph = SymmetryTools.buildSymmetryGraph(
				Arrays.asList(selfAlignment), atoms, true);
			...
			if (!alreadySeen.contains(i)) {
				...
				while (residue != -1 && !alreadySeen.contains(residue)) {
					...
					if (neigh.size() > 1) {
						...
						residue = -1; // residue does not have a next
				}
