...
public class PDBStatus {
    ...
    public static List<String> getReplacement(String oldPdbId, boolean recurse, boolean includeObsolete) {
        List<Map<String, String>> attrList = getStatusIdRecords(oldPdbId);
        //Expect a single record
        ...
        switch (status) {
            ...
            default: { //TODO handle other cases explicitly. They might have other syntax than "replacedBy"
                ...
                for (String replacement : replacements) {
                    ...
                    // Return the replacement.
                    if (recurse) {
                        mergeReversed(results, PDBStatus.getReplacement(replacement, recurse, includeObsolete));
                    } else {
                        mergeReversed(results, Collections.singletonList(replacement));
                    }
                }
    ...
    private static List<Map<String, String>> getStatusIdRecords(String... pdbIDs) {
        ...
        for (String pdbId : pdbIDs) {
            ...
            //check the cache
            Map<String, String> theResult = recordsCache.get(pdbId);
            if (theResult != null) {
