 ...
 * structure. Performs a global superposition of the MultipleAlignment in case 
...
public class ReferenceSuperimposer implements MultipleSuperimposer {
	...
	/**
	 * Superimpose all structures in a MultipleAlignment to the reference 
	 * structure. The superposition is done for all individual BlockSets. If 
	 * there is only one BlockSet, the superposition is alse set in the 
	...
	public void superimpose(MultipleAlignment alignment) 
		...
		if (alignment.getBlockSets().size() < 1) {
			throw new IndexOutOfBoundsException(
					"No aligned residues, number of Blocks == 0.");
		}
		
		if (atomArrays.size() <= reference) {
			throw new IndexOutOfBoundsException(String.format(
					"Invalid reference structure: requested %d but "
					+ "only %d structures.",reference,atomArrays.size()));
		}
