...
public class CathInstallation implements CathDatabase{
	...
	private final Map<String, List<CathDomain>> pdbMap;
	private final Map<String, CathDomain> domainMap;
	private final Map<String, CathNode> cathTree;
	...
	public List<CathDomain> getByCategory(CathCategory category) {
		...
		CathNode node;
		for (CathNode cathNode : cathTree.values()) {
			if ( (node = cathNode).getCategory() == category ) {
	...
	public List<CathDomain> filterByCathCode(String query) {
		...
		List<CathDomain> matches = new ArrayList<>();
		for (CathDomain cathDomain : domainMap.values()) {
			if (cathDomain.getCATH().startsWith(query) ) {
				matches.add(cathDomain);
			}
		}
		return matches;
	}
	...
	public List<CathDomain> filterByNodeName(String query) {
		...
		CathNode node;
		for (CathNode cathNode : cathTree.values()) {
			if ( (node = cathNode).getDescription().startsWith(query) ) {
	...
	public List<CathDomain> filterByDescription(String query) {
		...
		List<CathDomain> matches = new ArrayList<>();
		for (CathDomain cathDomain : domainMap.values()) {
			if (cathDomain.getName().startsWith(query) ) {
				matches.add(cathDomain);
			}
		}
		return matches;
	...
	public List<CathDomain> getDomainsByNodeId(String nodeId) {
		...
		for (CathDomain description : domainMap.values()) {
...
/*    private void transposeDomainData() {
	...
	private void parseCathDomall(BufferedReader bufferedReader) throws IOException{
		...
		while ( ((line = bufferedReader.readLine()) != null) ) {
			...
			int numberOfDomains = Integer.parseInt( token[1].substring(1) );
			int numberOfFragments = Integer.parseInt( token[2].substring(1) );
			int domIdx = 3;
			int segIdx;
			int sstop;
			int sstart;
...
//                If there is more than one domain, they are always numbered from 01.
				...
				int numberOfSegments = Integer.parseInt( token[domIdx] );
...
//        System.out.println("downloading " + remoteURL + " to: " + localFile);
		...
		File tempFile  = File.createTempFile(Download.getFilePrefix(localFile), "."+ Download.getFileExtension(localFile));
		...
		Download.copy(tempFile,localFile);
