...
import static org.biojava3.ws.alignment.qblast2.enums.BlastParameter.CMD;
import static org.biojava3.ws.alignment.qblast2.enums.BlastParameter.DATABASE;
import static org.biojava3.ws.alignment.qblast2.enums.BlastParameter.EMAIL;
...
import static org.biojava3.ws.alignment.qblast2.enums.BlastParameter.QUERY;
import static org.biojava3.ws.alignment.qblast2.enums.BlastParameter.RID;
import static org.biojava3.ws.alignment.qblast2.enums.BlastParameter.TOOL;
...
import org.biojava3.ws.alignment.qblast2.utils.MapToStringTransformer;
 ...
 */
public class NCBIQBlastService2 implements RemotePairwiseAlignmentService {
	/**
	 * Number of milliseconds by which expected job execution time is
	 * incremented if it is not finished yet. Calls to
	 * {@link #isReady(String, long)} method will return false until at least
	...
	private static final MapToStringTransformer MAP_TO_STRING = new MapToStringTransformer();
	...
	private Map<String, NCBIQBlastJob> jobs = new HashMap<String, NCBIQBlastJob>();

	public NCBIQBlastService2() throws Exception {
		try {
			serviceUrl = new URL(SERVICE_URL);
		} catch (MalformedURLException e) {
			throw new Exception("It looks like the URL for NCBI QBlast service is wrong. Cause: " + e.getMessage(), e);
		}
	}
	 ...
	 * Uses all of the parameters specified in {@code alignmentProperties}
	 * (parameters PROGRAM and DATABASE are required).
	 * 
	 * @param sequence : a {@code String} representing a sequence or Genbank ID
	 * @param alignmentProperties : a {@code RemotePairwiseAlignmentProperties}
	 *            object representing alignment properties
	 * @return the request id for this sequence, necessary to fetch results
	 *         after completion
	 * @throws Exception if unable to connect to the NCBI QBlast service or if
	 *             no sequence or required parameters PROGRAM and DATABASE are
	 *             not set
	 */
	@Override
	public String sendAlignmentRequest(String sequence, RemotePairwiseAlignmentProperties alignmentProperties) throws Exception {
		...
		params.put(FORMAT_TYPE, BlastOutputFormat2.HTML.getValue());
		params.put(TOOL, getTool());
		params.put(EMAIL, getEmail());

		String putCmd = MAP_TO_STRING.transform(params);
		...
		try {
			...
			NCBIQBlastJob job = new NCBIQBlastJob();
			String line;
			while ((line = reader.readLine()) != null) {
	 ...
	 * {@code present} property. See that method for more detailed description
	 * 
	 ...
	 * <p>
	 * If expected execution time (RTOE) is available for request, this method
	 * will always return false until that time passes. This is done to prevent
	...
	public boolean isReady(String id, long present) throws Exception {
		...
		arguments.put(CMD, "Get");
		arguments.put(RID, job.getId());

		OutputStreamWriter writer = null;
		BufferedReader reader = null;
		try {
			String checkRequest = MAP_TO_STRING.transform(arguments);
			URLConnection serviceConnection = setQBlastServiceProperties(serviceUrl.openConnection());
			writer = new OutputStreamWriter(serviceConnection.getOutputStream());
	...
	/**
	 * Extracts the actual Blast report for given request id according to
	 * options provided in {@code outputProperties} argument.
	 * <p>
	 * If the results are not ready yet, sleeps until they are available. If
	 * sleeping is not desired, call this method after {@code isReady} returns
	 * true
	 * 
	 * @param id : request id, which was returned by
	 *            {@code sendAlignmentRequest} method
	...
	public InputStream getAlignmentResults(String id, RemotePairwiseAlignmentOutputProperties outputProperties) throws Exception {
		...
		params.put(TOOL, getTool());
		params.put(EMAIL, getEmail());
		String getCmd = MAP_TO_STRING.transform(params);
	...
	/**
	 * Sends a delete request for given request id. Can be used after results of
	 * given search are no longer needed to be kept on Blast server
	 * 
	 * @param id request id, as returned by {@code sendAlignmentRequest} method
	 * @throws Exception if it is not possible to send the request
	 */
	public void sendDeleteRequest(String id) throws Exception {
		Map<String, String> params = new HashMap<String, String>();
		...
		OutputStreamWriter writer = null;
		try {
			String deleteRequest = MAP_TO_STRING.transform(params);
			URLConnection serviceConnection = setQBlastServiceProperties(serviceUrl.openConnection());
			writer = new OutputStreamWriter(serviceConnection.getOutputStream());
			writer.write(deleteRequest);
			writer.flush();
		} catch (IOException e) {
			throw new Exception("Could not send delete request. Cause: " + e.getMessage(), e);
		} finally {
			IOUtils.close(writer);
		}
	}
	...
	private URLConnection setQBlastServiceProperties(URLConnection conn) {
		...
		conn.setRequestProperty("User-Agent", "Biojava/NCBIQBlastService2");
